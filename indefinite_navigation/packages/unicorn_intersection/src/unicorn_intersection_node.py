...
class UnicornIntersectionNode:
    ...
    def cbIntersectionGo(self, msg):
        ...
        rospy.loginfo("This is the value of self.active [%s]", self.active)
        rospy.loginfo("This is the value of msg.data [%s]", msg.data)
        #if not self.active:
        #    rospy.loginfo("It goes into the first if")
        #    return

        if not msg.data:
            rospy.loginfo("It goes into the second if")
            return

        rospy.loginfo("This is the value of turn_type [%s]", self.turn_type)
        while self.turn_type == -1:
            if not self.active:
                rospy.loginfo("It goes into the while")
                return
            rospy.loginfo(
        ...
        tag_id = self.tag_id
        #self.turn_type = self.sub_turn_type.turn_type
        turn_type = self.turn_type
        #rospy.set_param("This is the turn type: " + turn_type)
        ...
        msg_done.data = True
        print("I think it is not entering here, but let's find out")
