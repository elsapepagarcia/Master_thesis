...
from duckietown_msgs.msg import FSMState, LanePose, SegmentList, Twist2DStamped, WheelEncoderStamped, BoolStamped
...
class LaneFilterNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        self.encoder_pub_right = rospy.Publisher("~right_wheel_encoder_node/tick", WheelEncoderStamped, queue_size=1)
        self.encoder_pub_left = rospy.Publisher("~left_wheel_encoder_node/tick", WheelEncoderStamped, queue_size=1)
        ...
        # Call the function to reset the encoders to 0 on initialization
        self.reset_encoders()
    ...
    def reset_encoders(self):
        # Create a WheelEncoderStamped message with zero values
        zeroed_encoder = WheelEncoderStamped()
        zeroed_encoder.data = 0
        #zeroed_encoder.header.stamp = rospy.Time.now()

        # Publish the zeroed encoder values
        self.encoder_pub_right.publish(zeroed_encoder)
        self.encoder_pub_left.publish(zeroed_encoder)

        rospy.loginfo("Encoders reset to 0")
    ...
    def cbProcessSegments(self, segment_list_msg):
        ...
        if self.currentVelocity:
            ...
            # KEEP THIS #
            #if self.lane_follow and self.intersection and (time.time() - self.time_calc) > 2:
            ...
            print("This is the value of the lane flag: " + str(self.lane_follow))
            print("This is the value of the intersection flag: " + str(self.intersection))
